...
int get_botID2(int bot, int sockertnumber) {
  ...
  /* !! ALLERT !! Here it should be client_sock[bot] itself !!! */
  for (int i = 0; i < recv(sockertnumber, client_message, 1024, 0); i++) {
...
void get_rpldata(int botid) {

  int ret = 0;
  char *value = NULL;
  char data;
  char src = 0x00;
  int client_index = get_index(botid);

  data = GET_RPL;
  create_packet(src, botid, sizeof(data), &data);
  memset(client_message, '\0', 1024);
...
void create_packet(int src, int dst, char length, char *data) {
  ...
  packet[PACKET_INTERNAL_CMD_LOC + 1] =
      0b000 << 5 | 0b1 << 4 | 0b0 << 3 | 0b000;
